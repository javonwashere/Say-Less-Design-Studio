...
const works = (collectionOfWorks) => {
  ...
  const workRows = collectionOfWorks.map(
    ({ client, slug, featuredImage }, key) => {
      const indexNumber =
        (key + 1).toString().length > 1 ? key + 1 : `0${key + 1}`
      ...
      return (
        <div className="work-row" id={key}>
          <div className="work-client" id={key}>
            <Link
          ...
          <div className="work-index" id={key}>
            {indexNumber}
          </div>
        </div>
      )
  ...
  useEffect(() => {
    imageContainer.current.addEventListener(
      'mousemove',
      function (event) {
